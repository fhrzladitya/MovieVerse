    ...
    <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-md">
      ...
      <div
        className={`relative h-screen w-screen overflow-y-auto ${
          isDark ? "bg-[#050505] text-white" : "bg-white text-slate-950"
        }`}
      >
        <button
          type="button"
          onClick={onClose}
          className="fixed right-4 top-4 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-red-500 text-lg font-bold text-white shadow-2xl transition hover:scale-110 hover:bg-red-600"
        >
          ✕
        </button>

        <div className="relative min-h-screen">
          <div className="relative h-[50vh] w-full md:h-[75vh]">
            <img
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12">
              <h2 className="mb-5 text-4xl font-black text-white md:text-7xl">
                {movie.name}
              </h2>

              <div className="flex flex-wrap gap-3">
                <span className="rounded-full bg-yellow-400 px-5 py-2 text-sm font-extrabold text-black">
                  {text.rating} {movie.rating.average}
                </span>
                ...
                {movie.genres.map((genre) => (
                  <span
                    key={genre}
                    className="rounded-full bg-red-500/80 px-5 py-2 text-sm font-bold text-white backdrop-blur-xl"
                  >
                    {getGenreLabel(genre)}
                  </span>
                ))}
          ...
          <div className="mx-auto max-w-6xl px-5 py-10 md:px-10">
            ...
            >
              ...
              <div className="overflow-hidden rounded-3xl border border-white/10 shadow-2xl">
                <iframe
                  className="h-[240px] w-full sm:h-[360px] md:h-[500px] lg:h-[650px]"
